"""
A minimal trainer class built on Pytorch Ignite with
...
class AudioTrainer(BaseTrainer):
    @torch.no_grad()
    def save_samples(self, generator, val_data, val_idx):
        self.print("Saving audio samples to tensorboard")

        samples = [val_data[idx] for idx in val_idx]
        batch = val_data.collate(samples)
        batch = audiotools.util.prepare_batch(batch, "cuda")
        clean, noisy = self.apply_transform(val_data.transform, batch)

        generator.eval()
        enhanced = generator(noisy.audio_data, noisy.sample_rate)
        enhanced = audiotools.AudioSignal(enhanced, noisy.sample_rate)
